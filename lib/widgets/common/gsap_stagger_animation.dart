...
class _GSAPEnhancedAnimationState extends State<GSAPEnhancedAnimation> {
  ...
  final ValueNotifier<double> _translateX = ValueNotifier<double>(0.0);
  ...
  void initState() {
    ...
    _opacity.value = 1.0;
    _translateX.value = 0.0;
    _translateY.value = 0.0;
    _scale.value = 1.0;
    _rotation.value = 0.0;
    ...
    try {
      if (event != null && event.detail != null) {
        ...
        if (elementId?.toString() == widget.elementId && mounted) {
          ...
          // Directly update ValueNotifiers to avoid parent rebuild/jitter
          final dynamic opacityValue = detail['opacity'];
          if (opacityValue != null) {
            _opacity.value = (opacityValue as num).toDouble();
          } else if (!_gsapReady) {
            _opacity.value = 1.0;
          }

          final dynamic xValue = detail['x'];
          if (xValue != null) _translateX.value = (xValue as num).toDouble();

          final dynamic yValue = detail['y'];
          if (yValue != null) _translateY.value = (yValue as num).toDouble();

          final dynamic scaleValue = detail['scale'];
          if (scaleValue != null) {
            _scale.value = (scaleValue as num).toDouble();
          } else if (!_gsapReady) {
            _scale.value = 1.0;
          }
